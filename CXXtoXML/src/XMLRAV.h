...
public:
  ...
  virtual bool BridgeNestedNameSpecifierLoc(
      clang::NestedNameSpecifierLoc NNS) = 0;
  virtual bool BridgeDeclarationNameInfo(
      clang::DeclarationNameInfo NameInfo) = 0;
  ...
  virtual bool BridgeTemplateArgumentLoc(
      const clang::TemplateArgumentLoc &ArgLoc) = 0;
  // virtual bool BridgeTemplateArguments(const TemplateArgument *Args,
  // unsigned NumArgs) = 0;
  virtual bool BridgeConstructorInitializer(
      clang::CXXCtorInitializer *Init) = 0;
  // virtual bool BridgeLambdaCapture(LambdaExpr *LE, const LambdaCapture *C) =
  // 0;
